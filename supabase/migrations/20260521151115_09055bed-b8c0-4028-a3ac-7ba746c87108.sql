...
ALTER TABLE public.tabs ADD COLUMN IF NOT EXISTS manual_discount_item_amounts jsonb;
...
tab_revenues AS (
  SELECT ti.id, t.establishment_id, 'revenue'::finance_entry_type AS type,
    round(
      CASE
        -- Per-item explicit manual discount overrides proportional distribution
        WHEN t.discount_type = 'manual'
             AND t.manual_discount_item_amounts IS NOT NULL
             AND (t.manual_discount_item_amounts ? ti.id::text)
          THEN GREATEST(0, ti.total_price - COALESCE((t.manual_discount_item_amounts ->> ti.id::text)::numeric, 0))
        WHEN t.discount_type = 'manual' AND t.manual_discount_item_amounts IS NOT NULL
          THEN ti.total_price
