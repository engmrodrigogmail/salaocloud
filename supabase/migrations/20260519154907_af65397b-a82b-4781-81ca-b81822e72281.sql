-- Consolida despesas de comissão na view financeira:
-- comissões PAGAS no mesmo dia para o mesmo profissional viram UMA única
-- linha somada. Comissões pendentes continuam individuais.
...
-- Comissões pendentes (não agrupadas)
  ...
  SELECT
    ...
    CASE
      WHEN COALESCE((SELECT fs.commission_expense_trigger FROM public.finance_settings fs WHERE fs.establishment_id = pc.establishment_id), 'on_tab_close')
        ...
        THEN (pc.created_at AT TIME ZONE 'America/Sao_Paulo')::date
...
-- Comissões PAGAS agrupadas por (estabelecimento, profissional, dia do pagamento)
commission_paid_grouped AS (
  SELECT
    -- id sintético determinístico (mesmo grupo => mesmo id)
    md5(
...
-- Comissões com trigger antigo (on_tab_close) — comportamento original individual
