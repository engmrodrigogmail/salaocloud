...
BEGIN
  ...
  -- Temp table to stage raw (unrounded) commission amounts for later reconciliation
  ...
  -- Reconciliation: ensure sum of rounded commissions per (professional, rule)
  -- equals round(sum(raw), 2). Adjusts the largest row in each bucket.
  FOR _bucket IN
    SELECT
      s.professional_id,
      s.rule_key,
...
-- Reconcile the existing tab the user reported
SELECT public.recalculate_tab_commissions('404fcb84-8ae6-4bfa-a900-656018d1c1ae'::uuid);
